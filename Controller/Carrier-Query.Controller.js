...
const Model = require('../Models/Carrier-Query.Model')
const mongoose = require('mongoose')
const ModelName = 'CarrierQuery'

module.exports = {

// carrier-query.controller.js
create: async (req, res, next) => {
  try {
      const data = req.body;

      if (data.resumePath) {
          data.resume = data.resumePath;  // ✅ Save full file path like "2025/5/filename.pdf"
      }
  ...
  delete: async (req, res, next) => {
    try {
        ...
        const carrierQuery = await Model.findById(new mongoose.Types.ObjectId(id)); // Use new here
        if (!carrierQuery) {
            return res.status(404).json({ message: 'Query not found' });
        }

        // Mark the Query as inactive instead of deleting
        ...
        res.json({ success: true, message: 'Query marked as inactive', data: result });
