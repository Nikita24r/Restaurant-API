...
module.exports = {
  ...
  delete: async (req, res, next) => {
    try {
        ...
        const courses = await Model.findById(new mongoose.Types.ObjectId(id)); // Use new here
        if (!courses) {
            return res.status(404).json({ message: 'courses not found' });
        }

        // Mark the courses as inactive instead of deleting
        ...
        res.json({ success: true, message: 'courses marked as inactive', data: result });
